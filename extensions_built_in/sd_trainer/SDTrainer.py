...
class SDTrainer(BaseSDTrainProcess):
    ...
    def __init__(self, process_id: int, job, config: OrderedDict, **kwargs):
        ...
        if self.train_config.blank_prompt_preservation:
            if self.network_config is None:
                raise ValueError(
                    "blank_prompt_preservation requires a network to be set"
                )

        if (
            self.train_config.blank_prompt_preservation
            or self.train_config.diff_output_preservation
        ):
        ...
        if isinstance(self.train_config.guidance_loss_target, (int, float)):
            self._guidance_loss_target_batch = float(
                self.train_config.guidance_loss_target
            )
            ...
            raise ValueError(
                f"Unknown guidance loss target type {type(self.train_config.guidance_loss_target)}"
            )

        # Grad norm logging (optional): L=1 logs scalars per step; L>1 buffers on GPU, logs aggregates.
        self._grad_norm_buf_pre_tensors: List[torch.Tensor] = []
        self._grad_norm_buf_clip_max: List[float] = []
        self._grad_norm_opt_count: int = 0
        self._grad_norm_next_flush_k: int = max(
            1, int(getattr(self.train_config, "grad_norm_log_every", 1))
        )

    def _grad_norm_log_enabled(self) -> bool:
        return bool(getattr(self.train_config, "log_grad_norm_stats", False))

    @staticmethod
    def _grad_norm_next_flush_target(opt_count: int, log_every: int) -> int:
        """Next optimizer-step index (inclusive) at which to flush the GPU buffer."""
        L = max(1, int(log_every))
        if L <= 1:
            return opt_count + 1
        k = int(opt_count)
        if k % L == 0:
    ...
    def _maybe_flush_grad_norm_buffer_for_clip_cap_change(self) -> None:
        if not self._grad_norm_log_enabled():
            return
        L = int(getattr(self.train_config, "grad_norm_log_every", 1))
        if L <= 1 or not self._grad_norm_buf_pre_tensors:
            return
        new_cap = float(self.train_config.max_grad_norm)
        old_cap = float(self._grad_norm_buf_clip_max[0])
        if abs(new_cap - old_cap) <= 1e-12:
            return
        self._flush_grad_norm_gpu_buffer()

    def _flush_grad_norm_gpu_buffer(self) -> None:
        """Aggregate buffered pre-clip norms on GPU; write float stats to _last_optimizer_metrics."""
        if not self._grad_norm_buf_pre_tensors:
            return
        pres = (
            torch.stack([t.reshape(1) for t in self._grad_norm_buf_pre_tensors])
            .reshape(-1)
            .float()
        )
        cmx = torch.tensor(
            self._grad_norm_buf_clip_max, device=pres.device, dtype=pres.dtype
        )
        clip_mask = cmx > 0
        clipped = torch.zeros_like(pres)
        if bool(clip_mask.any().item()):
        ...
        self._grad_norm_next_flush_k = self._grad_norm_next_flush_target(
            self._grad_norm_opt_count, L
        )
    ...
    def flush_grad_norm_log_buffers(self) -> None:
        ...
        if L <= 1:
            return
        if self._grad_norm_buf_pre_tensors:
            self._flush_grad_norm_gpu_buffer()
            ...
            for i in range(len(self.sample_config.prompts)):
                ...
                if self.sd.encode_control_in_text_embeddings and has_control_images:
                    ...
                    if gen_img_config.ctrl_img_1 is not None:
                        ctrl_img_1 = Image.open(gen_img_config.ctrl_img_1).convert(
                            "RGB"
                        )
                        # convert to 0 to 1 tensor
                        ctrl_img_1 = (
                    ...
                    if gen_img_config.ctrl_img_2 is not None:
                        ctrl_img_2 = Image.open(gen_img_config.ctrl_img_2).convert(
                            "RGB"
                        )
                        # convert to 0 to 1 tensor
                        ctrl_img_2 = (
                    ...
                    if gen_img_config.ctrl_img_3 is not None:
                        ctrl_img_3 = Image.open(gen_img_config.ctrl_img_3).convert(
                            "RGB"
                        )
                        # convert to 0 to 1 tensor
                        ctrl_img_3 = (
                    ...
                    positive = self.sd.encode_prompt(
                        gen_img_config.prompt, control_images=ctrl_img
                    ).to("cpu")
                    negative = self.sd.encode_prompt(
                        gen_img_config.negative_prompt, control_images=ctrl_img
                    ).to("cpu")
                else:
                    positive = self.sd.encode_prompt(gen_img_config.prompt).to("cpu")
                    negative = self.sd.encode_prompt(gen_img_config.negative_prompt).to(
                        "cpu"
                    )

                self.sd.sample_prompts_cache.append(
                    {"conditional": positive, "unconditional": negative}
                )
    ...
    def hook_before_train_loop(self):
        ...
        if self.train_config.diffusion_feature_extractor_path is not None:
            ...
            #     vae = self.sd.vae
            self.dfe = load_dfe(
                self.train_config.diffusion_feature_extractor_path, vae=vae, sd=self.sd
            )
    ...
    def calculate_loss(
        ...
        **kwargs,
        ...
        if self.train_config.correct_pred_norm or (
            ...
            if (
                ...
                with torch.no_grad():
                    ...
                    if len(noise_pred.shape) == 5:
                        prior_mask = prior_mask.unsqueeze(
                            2
                        )  # add time dimension back for video
            ...
            if not self.sd.is_flow_matching:
                raise ValueError(
                    "Signal amplification is only supported for flow matching models"
                )
            with torch.no_grad():
            ...
            if self.dfe.version == 1:
                ...
                if model is not None and hasattr(model, "get_stepped_pred"):
                    ...
                    for idx in range(bs):
                        ...
                        sigma = self.sd.noise_scheduler.sigmas[
                            self.sd.noise_scheduler.step_index
                        ]
                        sigma_next = self.sd.noise_scheduler.sigmas[
                            -1
                        ]  # use last sigma for final step
                        prev_sample = sample + (sigma_next - sigma) * model_output
                        stepped_chunks.append(prev_sample)

                    stepped_latents = torch.cat(stepped_chunks, dim=0)

                stepped_latents = stepped_latents.to(
                    self.sd.vae.device, dtype=self.sd.vae.dtype
                )
                sl = stepped_latents
                if len(sl.shape) == 5:
                    # video B,C,T,H,W
                ...
                with torch.no_grad():
                    ...
                    dfe_scaler = 1 - (timesteps.float() / 1000.0).view(-1, 1, 1, 1).to(
                        self.device_torch
                    )
        ...
        if self.train_config.do_guidance_loss:
            with torch.no_grad():
                ...
                if self.train_config.do_guidance_loss_cfg_zero:
                    ...
                    alpha = (
                        alpha.view(batch_size, 1, 1, 1)
                        if not is_video
                        else alpha.view(batch_size, 1, 1, 1, 1)
                    )
                else:
                    alpha = 1.0
                ...
                if isinstance(guidance_scale, list):
                    guidance_scale = torch.tensor(guidance_scale).to(
                        target.device, dtype=target.dtype
                    )
                    guidance_scale = (
                        guidance_scale.view(-1, 1, 1, 1)
                        if not is_video
                        else guidance_scale.view(-1, 1, 1, 1, 1)
                    )
        ...
        try:
            if len(noise_pred.shape) == 5:
                # video B,C,T,H,W
                mask_multiplier = mask_multiplier.unsqueeze(
                    2
                )  # add time dimension back for video
                mask_multiplier = mask_multiplier.repeat(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            lerp_vector = (
                ...
                + noise_sample * t_frac[:, None, None, None]
            )
        ...
        with torch.no_grad():
            ...
            lerp_perturbed = (
                ...
                + noise_sample * t_frac_plus_eps[:, None, None, None]
            )
            timesteps_cat_perturbed = (
                torch.cat([t_frac_plus_eps, r_frac], dim=0) * total_steps
            )
        ...
        # compute loss
        loss = torch.nn.functional.mse_loss(
            u_shifted.float(), v_target.float(), reduction="none"
        )
        ...
        with torch.no_grad():
            ...
            if (
                ...
                if batch.prompt_embeds is not None:
                    embeds_to_use = batch.prompt_embeds.clone().to(
                        self.device_torch, dtype=dtype
                    )
                else:
                    prompt_kwargs = {}
                    if (
                        self.sd.encode_control_in_text_embeddings
                    ...
                    embeds_to_use = (
                        ...
                        .to(self.device_torch, dtype=dtype, **prompt_kwargs)
                        .detach()
                    )
    ...
    def train_single_accumulation(self, batch: DataLoaderBatchDTO):
        with torch.no_grad():
                ...
                if self.negative_prompt_pool is not None:
                    ...
                    for i in range(noisy_latents.shape[0]):
                        num_neg = random.randint(
                            1, self.train_config.max_negative_prompts
                        )
                        this_neg_prompts = [
                            random.choice(self.negative_prompt_pool)
                    ...
                    self.batch_negative_prompt = [
                        "" for _ in range(batch.latents.shape[0])
                    ]
            ...
            if any(
                [batch.file_items[idx].is_reg for idx in range(len(batch.file_items))]
            ):
                has_clip_image = True
            ...
            if any(
                [
                    batch.file_items[idx].prior_reg
                    for idx in range(len(batch.file_items))
                ]
            ):
                do_reg_prior = True
        ...
        for (
            ...
            with network:
                    ...
                    if (
                        ...
                        with torch.set_grad_enabled(False):
                            if batch.prompt_embeds is not None:
                                ...
                                if (
                                    self.cached_trigger_embeds is not None
                                    and not is_reg
                                ):
                                    embeds_to_use = (
                ...
                if self.train_config.do_guidance_loss and isinstance(
                    self.train_config.guidance_loss_target, list
                ):
                    batch_size = noisy_latents.shape[0]
                ...
                if batch.unconditional_latents is not None or self.do_guided_loss:
                    ...
                    if (
                        ...
                        with torch.no_grad():
                            if self.train_config.diff_output_preservation:
                                preservation_embeds = self.diff_output_preservation_embeds.expand_to_batch(
                                    noisy_latents.shape[0]
                                )
                        ...
                        preservation_pred = self.predict_noise(
                            ...
                            timesteps=timesteps,
                            conditional_embeds=preservation_embeds.to(
                                self.device_torch, dtype=dtype
                            ),
                            unconditional_embeds=unconditional_embeds,
                            batch=batch,
    ...
    def hook_train_loop(
        ...
        self._maybe_flush_grad_norm_buffer_for_clip_cap_change()
        ...
        for batch in batch_list:
            if self.sd.is_multistage:
                ...
                if (
                    ...
                    or self.current_boundary_index
                    not in self.sd.trainable_multistage_boundaries
                ):
                    # iterate to make sure we only train trainable_multistage_boundaries
                    while True:
                        self.steps_this_boundary = 0
                        self.current_boundary_index += 1
                        if self.current_boundary_index >= len(
                            self.sd.multistage_boundaries
                        ):
                            self.current_boundary_index = 0
                        if (
                            self.current_boundary_index
                            in self.sd.trainable_multistage_boundaries
                        ):
        ...
        if not self.is_grad_accumulation_step:
            ...
            if self.train_config.optimizer != "adafactor":
                ...
                pre_clip_tensors: List[torch.Tensor] = []
                if isinstance(self.params[0], dict):
                    for i in range(len(self.params)):
                        ...
                        if n is not None:
                            pre_clip_tensors.append(n.detach().float().reshape(1))
                else:
                    n = self.accelerator.clip_grad_norm_(
                    ...
                    if n is not None:
                        pre_clip_tensors.append(n.detach().float().reshape(1))
                if pre_clip_tensors:
                    gn_tensor = torch.max(torch.cat(pre_clip_tensors, dim=0)).reshape(1)
                    if self._grad_norm_log_enabled():
                        L = max(
                            1, int(getattr(self.train_config, "grad_norm_log_every", 1))
                        )
                        if L <= 1:
                            gn_pre = float(gn_tensor.cpu().item())
                            self._last_optimizer_metrics["grad_norm_pre"] = gn_pre
                            if max_norm > 0.0:
                                self._last_optimizer_metrics["grad_norm_post"] = min(
                                    gn_pre, max_norm
                                )
                            else:
                                self._last_optimizer_metrics["grad_norm_post"] = gn_pre
                        else:
                            self._grad_norm_buf_pre_tensors.append(
                                gn_tensor.detach().to(dtype=torch.float32)
                            )
                            self._grad_norm_buf_clip_max.append(max_norm)
                            self._grad_norm_opt_count += 1
                            if (
                                len(self._grad_norm_buf_pre_tensors) > 0
                                and self._grad_norm_opt_count
                                >= self._grad_norm_next_flush_k
                            ):
                                self._flush_grad_norm_gpu_buffer()
        ...
        if getattr(self, "_is_final_train_step", False):
