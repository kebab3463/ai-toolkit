...
class TrainConfig:
    def __init__(self, **kwargs):
        ...
        self.attention_backend: str = kwargs.get(
            "attention_backend", "native"
        )  # native, flash, _flash_3_hub, _flash_3,
        # Optional per-module overrides. By default, keep VAE/text-encoder unchanged.
        self.attention_backend_vae: str = kwargs.get("attention_backend_vae", "native")
        self.attention_backend_text_encoder: str = kwargs.get(
        ...
        self.merge_network_on_save_strength = kwargs.get(
        ...
        self.do_signal_correction_noise = kwargs.get(
            "do_signal_correction_noise", False
        )
        ...
        self.batch_noise_correction_scale = kwargs.get(
            "batch_noise_correction_scale", 0.1
        )
        self.do_signal_amplification = kwargs.get("do_signal_amplification", False)
        self.signal_amplification_strength = kwargs.get(
            "signal_amplification_strength", 0.5
        )

        self.signal_correction_noise_scale = kwargs.get(
            "signal_correction_noise_scale", 1.0
        )
        ...
        self.blank_prompt_preservation_multiplier = kwargs.get(
        ...
        self.do_guidance_loss_cfg_zero: bool = kwargs.get(
        ...
        self.differential_guidance_scale = kwargs.get(
...
class TrainStageConfig:
    def __init__(self, **kwargs):
        ...
        self.lr: Optional[float] = kwargs.get("lr", None)
...
class ModelConfig:
    def __init__(self, **kwargs):
        ...
        self.layer_offloading_transformer_percent = kwargs.get(
            "layer_offloading_transformer_percent", 1.0
        )
        self.layer_offloading_text_encoder_percent = kwargs.get(
...
class DatasetConfig:
    ...
    def __init__(self, **kwargs):
        ...
        self.do_audio: bool = kwargs.get(
            "do_audio", False
        )  # load audio from video files for models that support it
        self.audio_preserve_pitch: bool = kwargs.get(
            "audio_preserve_pitch", False
        )  # preserve pitch when stretching audio to fit num_frames
        self.audio_normalize: bool = kwargs.get(
            "audio_normalize", False
        )  # normalize audio volume levels when loading
...
class GenerateImageConfig:
    def __init__(
        ...
        ctrl_img_2: Optional[
            str
        ] = None,  # second control image for multi control model
...
def validate_configs(
    ...
    if model_config.accuracy_recovery_adapter is not None:
        if model_config.assistant_lora_path is not None:
            raise ValueError(
                "Cannot use accuracy recovery adapter and assistant lora at the same time. "
    ...
    if model_config.arch == "qwen_image_edit":
        if train_config.unload_text_encoder:
            raise ValueError(
                "Cannot cache unload text encoder with qwen_image_edit model. Control images are encoded with text embeddings. You can cache the text embeddings though"
            )

    if train_config.diff_output_preservation and train_config.blank_prompt_preservation:
        raise ValueError(
            "Cannot use both differential output preservation and blank prompt preservation at the same time. Please set one of them to False."
        )

    if train_config.batch_size > 1 and any(
        dataset_config.auto_frame_count for dataset_config in dataset_configs
    ):
        raise ValueError(
            "Cannot use batch size greater than 1 with auto_frame_count. Please set batch_size to 1 or auto_frame_count to False."
        )
